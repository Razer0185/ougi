...
/** Must match desktop/OugiHost/SealSecrets.cs (rewritten every seal with a fresh key). */
let PASSPHRASE = 'OugiHost.Runtime.Seal.v1.dev-only';

function writeSealSecretsCs(passphrase) {
  const safe = String(passphrase).replace(/\\/g, '\\\\').replace(/"/g, '\\"');
  const cs = `// AUTO-GENERATED by scripts/seal-runtime.js — do not commit real pack secrets.
namespace OugiHostApp
{
    internal static class SealSecrets
    {
        public const string Passphrase = "${safe}";
    }
}
`;
  fs.writeFileSync(path.join(ROOT, 'desktop', 'OugiHost', 'SealSecrets.cs'), cs, 'utf8');
}
...
function stageRuntime(nodeDir, version) {
  ...
  // Marker: sealed PC builds must pass license heartbeat (see agent/pc-entry.js + index.js)
  fs.writeFileSync(
    path.join(runtimeDir, 'SEALED_BUILD.json'),
    JSON.stringify(
      {
        product: 'Ougi',
        sealed: true,
        version,
        note: 'Requires active PC Host subscription. Do not redistribute.',
      },
      null,
      2
    ) + '\n',
...
async function main() {
  ...
  // Unique key every pack — cracking one build does not unlock the next
  PASSPHRASE = crypto.randomBytes(32).toString('base64url');
  writeSealSecretsCs(PASSPHRASE);
  log('Generated per-build seal key (embedded into OugiHost.exe at publish)');
